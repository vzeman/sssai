...
class AssetInventory(Base):
    """Tracks technologies detected during scans, grouped by target."""
    __tablename__ = "asset_inventory"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id: Mapped[str] = mapped_column(ForeignKey("users.id"), index=True)
    scan_id: Mapped[str] = mapped_column(ForeignKey("scans.id", ondelete="SET NULL"), nullable=True)
    target: Mapped[str] = mapped_column(String, index=True)
    technology_name: Mapped[str] = mapped_column(String, index=True)
    technology_version: Mapped[str | None] = mapped_column(String, nullable=True)
    cpe_entries: Mapped[list | None] = mapped_column(JSON, nullable=True)  # CPE identifiers for NVD lookups
    first_seen: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
    last_seen: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())

    user: Mapped["User"] = relationship()
    cve_alerts: Mapped[list["CveAlert"]] = relationship(back_populates="asset", cascade="all, delete-orphan")


class CveAlert(Base):
    """CVE alerts generated by the background CVE monitoring job."""
    __tablename__ = "cve_alerts"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id: Mapped[str] = mapped_column(ForeignKey("users.id"), index=True)
    asset_id: Mapped[str] = mapped_column(ForeignKey("asset_inventory.id", ondelete="CASCADE"))
    cve_id: Mapped[str] = mapped_column(String, index=True)  # e.g. "CVE-2026-12345"
    technology_name: Mapped[str] = mapped_column(String)
    technology_version: Mapped[str | None] = mapped_column(String, nullable=True)
    cvss_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    cvss_severity: Mapped[str | None] = mapped_column(String, nullable=True)  # NONE, LOW, MEDIUM, HIGH, CRITICAL
    ...
    user: Mapped["User"] = relationship()
    asset: Mapped["AssetInventory"] = relationship(back_populates="cve_alerts")
